...
* The `uri` function MUST NOT be used to check for existence of a token type, It is possible for an implementation to return a valid string even if the tokenId does not exist, As such this method is only meaningful when called on existing token type
